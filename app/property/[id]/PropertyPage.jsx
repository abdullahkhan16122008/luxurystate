...
export default function PropertyPage({ params }) {
    ...
    let fetchProperty = async () => {
        ...
        try {
            let response = await axios.post(`${api}/api/get/property/details`, {_id: param.id});
            if(response.data.success){
                setProperty(response.data.property)
                console.log(response.data)
            } else if (!response.data.success) {
                return <div className="container mx-auto py-24 text-center text-2xl">Property not found</div>;
            }
        } catch (err) {
            console.log(err)
        }
